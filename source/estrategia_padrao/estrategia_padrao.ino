...
const int distancia_de_leitura_ultrassonico {
  35
};
...
void setup()
{
#if defined(_DEBUG_MICROSTART_) || defined(_DEBUG_SENSORES_) || defined(_DEBUG_MOVIMENTOS_) || defined(_DEBUG_LED_)
  Serial.begin(115200);
#endif
...
void loop()
{
  ...
  {
    if (IrReceiver.decodedIRData.decodedRawData == ready)
    {
      for (int j{0}; j < 3; j++)
      {
        #ifdef _DEBUG_LED_
        ...
        #endif
        digitalWrite(pino_led_ready, HIGH);
        delay(167); // delay de 1/6 de segundo pro LED piscar 3 vezes
        digitalWrite(pino_led_ready, LOW);
        delay(167);
      }
    }
  }
  ...
  if ((dist_meio != 0 && dist_meio < distancia_de_leitura_ultrassonico) && IrReceiver.decodedIRData.decodedRawData == start)
  {
...
void mudar_orientacao()
{
  parar(ESC_esquerdo, ESC_direito);
  delay(tempo_intervalo);
  if (dist_direita != 0 && dist_direita < distancia_de_leitura_ultrassonico && IrReceiver.decodedIRData.decodedRawData == start)
  {
  ...
  else if ((dist_esquerda != 0) && (dist_esquerda < distancia_de_leitura_ultrassonico) && IrReceiver.decodedIRData.decodedRawData == start) // Qual a diferença entre com e sem parênteses?
  {
